...
public class SleepService {
    ...
    public void makeSleep(SleepDto.Request request) {
        ...
        if (request.getAlarmInfo() != null) {
            isWakeUpAlarmSet = request.getAlarmInfo().getIsWakeUpAlarmSet();
            if (Boolean.TRUE.equals(isWakeUpAlarmSet) && request.getAlarmInfo().getAlarmSettings() != null) {
                SleepDto.AlarmSettingsDto dto = request.getAlarmInfo().getAlarmSettings();
                alarmSettings = AlarmSettings.builder()
                        .alarmSnoozeCnt(dto.getAlarmSnoozeCnt())
                        .timeToWakeUp(dto.getTimeToWakeUp())
                        .antiSleepMode(dto.getAntiSleepMode())
                        .build();
            }
        }
        ...
        Sleep sleep = Sleep.builder()
                ...
                .isWakeUpAlarmSet(isWakeUpAlarmSet)
    ...
    private double calculateSleepScore(Sleep sleep) {
        ...
        int snoozeCnt = (sleep.getAlarmSettings() != null && sleep.getAlarmSettings().getAlarmSnoozeCnt() != null)
                ? sleep.getAlarmSettings().getAlarmSnoozeCnt() : 0;
        double snoozePenalty = Math.max(1.0 - (0.05 * snoozeCnt), 0.5);

        int timeToWakeUp = (sleep.getAlarmSettings() != null && sleep.getAlarmSettings().getTimeToWakeUp() != null)
                ? sleep.getAlarmSettings().getTimeToWakeUp() : 0;
        double wakeUpPenalty = timeToWakeUp <= 10 ? 1.0 : Math.max(1.0 - (timeToWakeUp - 10) * 0.02, 0.6);

        double antiSleepBonus = (sleep.getAlarmSettings() != null && Boolean.TRUE.equals(sleep.getAlarmSettings().getAntiSleepMode()))
                ? 1.05 : 1.0;
    ...
    public SleepDto.Response updateSleep(SleepDto.Request request) {
        ...
        // AlarmInfo 업데이트
        AlarmSettings alarmSettings = existingSleep.getAlarmSettings();
        Boolean isWakeUpAlarmSet = existingSleep.getIsWakeUpAlarmSet();
    ...
    // ✅ 엔티티 → DTO 변환 메서드
    private SleepDto.Response toResponse(Sleep sleep) {
        SleepDto.AlarmSettingsDto alarmSettingsDto = null;
        if (sleep.getAlarmSettings() != null) {
            alarmSettingsDto = SleepDto.AlarmSettingsDto.builder()
                    .alarmSnoozeCnt(sleep.getAlarmSettings().getAlarmSnoozeCnt())
                    .timeToWakeUp(sleep.getAlarmSettings().getTimeToWakeUp())
                    .antiSleepMode(sleep.getAlarmSettings().getAntiSleepMode())
                    .build();
        }

        SleepDto.AlarmInfoDto alarmInfoDto = SleepDto.AlarmInfoDto.builder()
                .isWakeUpAlarmSet(sleep.getIsWakeUpAlarmSet())
                .alarmSettings(alarmSettingsDto)
                .build();
